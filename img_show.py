...
model = torch.load("model/effnetv2_s.pth",map_location=device)
...
def show_feature_map(
    ...
    for index in range(1, feature_map_num + 1):  # 通过遍历的方式，将64个通道的tensor拿出
        ...
        plt.axis('off')
        imageio.imsave('images//' + str(index) + ".png", feature_map[index - 1])
    plt.show()


if __name__ == '__main__':
    image_dir = "STIHDP_DEP0822/train/Buried PD/0_16_1.jpg"
    # 定义提取第几层的feature map
    k = 0
